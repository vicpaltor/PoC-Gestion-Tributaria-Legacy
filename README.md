...
1\.  `01\_setup\_users.sql`: Creación del esquema `GESTION\_TRIBUTARIA` en una PDB.

2\.  `02\_schema\_tables.sql`: Creación de tablas (`INMUEBLES`, `NOTIFICACIONES`) y secuencias.

3\.  `03\_procedures\_logic.sql`: Despliegue de los Procedimientos Almacenados.
...
Java: Valida el ID (Pre-Query).

Java: Invoca al Procedimiento PROCESO\_COMPLETO\_VALORACION.

Oracle: Calcula el valor según zona y metros.

Oracle: Inserta una notificación automática.

Java: Recupera y muestra los datos actualizados (Post-Query).
